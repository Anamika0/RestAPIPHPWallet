...
!empty($data->rd_amount) && !empty($data->rd_type) &&
!empty($data->rate_of_Interest) && !empty($data->duration)){
	...
	if($rd->update()){
		...
		echo json_encode(array("message" => "RD was updated."));
	}else{
		http_response_code(503);
		echo json_encode(array("message" => "Unable to update rd."));
	}

} else {
	http_response_code(400);
    echo json_encode(array("message" => "Unable to update rd. Data is incomplete."));
}
?>