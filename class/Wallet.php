...
class Wallet{   
	...
	function update_coins(){
		...
		//$sql1= "UPDATE wallet SET coin = coin- '{$coins}' WHERE walletId= {$from}";

 if(mysqli_query($this->conn, "UPDATE wallet SET coin = coin-'{$coins}' WHERE walletId= {$from}"))
	 
     	{ 
		
		if(mysqli_query($this->conn, "UPDATE wallet SET coin = coin+'{$coins}' WHERE walletId= {$to}")) {
